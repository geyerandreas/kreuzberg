...
/// 2. Title → SectionHeader remapping
/// 3. Full-page picture removal (>90% page area)
/// 4. Overlap resolution (IoU > 0.8 or containment > 0.8)
/// 5. Label-specific preference rules
/// 6. Cross-type overlap handling (KVR vs Table)
pub fn apply_heuristics(detections: &mut Vec<LayoutDetection>, page_width: f32, page_height: f32) {
    ...
    // 2. Remap Title → SectionHeader (Docling convention).
    for det in detections.iter_mut() {
        if det.class == LayoutClass::Title {
            det.class = LayoutClass::SectionHeader;
        }
    }

    // 3. Remove full-page pictures (>90% of page area).
    detections.retain(|d| {
    ...
    // 4. Overlap resolution — iterative (up to 3 passes).
    for _ in 0..3 {
    ...
    // 5. Cross-type overlap: remove KVR if 90%+ overlapping with Table and conf_diff < 0.1.
