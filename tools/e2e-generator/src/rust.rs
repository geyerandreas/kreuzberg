use crate::fixtures::{
    Assertions, EmbedAssertions, EmbedSpec, ExtractionMethod, Fixture, InputType, PluginTestSpec,
    RenderAssertions,
};
...
pub fn generate(fixtures: &[Fixture], output_root: &Utf8Path) -> Result<()> {
    ...
    if !render_fixtures.is_empty() {
        ...
        let path = tests_dir.join("render_test.rs");
        fs::write(&path, content).with_context(|| format!("Writing {}", path))?;
    }
...
fn generate_embed_tests_rust(fixtures: &[&Fixture]) -> Result<String> {
    ...
    writeln!(buf, "// To regenerate: cargo run -p kreuzberg-e2e-generator -- generate --lang rust")?;
    writeln!(buf, "\n// Tests for standalone embed() fixtures.")?;
    writeln!(buf, "#![allow(clippy::too_many_lines)]")?;
    writeln!(buf, "use e2e_rust::assertions;")?;
    writeln!(buf, "use kreuzberg::core::config::{{EmbeddingConfig, EmbeddingModelType}};")?;
    writeln!(buf, "use std::sync::Arc;")?;
    writeln!(buf)?;
...
fn render_embed_test(fixture: &Fixture) -> Result<String> {
    ...
    if !skip_platforms.is_empty() {
        let conditions = skip_platforms.iter().filter_map(|p| match p.as_str() {
    ...
    writeln!(code, "    let model = EmbeddingModelType::Preset(\"{model_name}\".to_string());")?;
    if let Some(bs) = batch_size {
        writeln!(code, "    let config = EmbeddingConfig {{\n        model,\n        normalize: {normalize},\n        batch_size: {bs},\n    }};")?;
    } else {
        writeln!(code, "    let config = EmbeddingConfig {{\n        model,\n        normalize: {normalize},\n        ..Default::default()\n    }};")?;
    }
    writeln!(code)?;

    let texts_literal = spec.texts.iter().map(|s| format!("\"{}\"", escape_rust_string(s))).collect::<Vec<_>>().join(", ");
    writeln!(code, "    let texts = vec![{texts_literal}];")?;

    if is_async {
        writeln!(code, "    let result_res = kreuzberg::embeddings::embed_texts_async(&texts, Arc::new(config)).await;")?;
    } else {
        writeln!(code, "    let result_res = kreuzberg::embeddings::embed_texts(&texts, &config);")?;
    }
    ...
    let count_lit = assertions.count.map(|c| format!("Some({c})")).unwrap_or_else(|| "None".into());
    let dim_lit = assertions.dimensions.map(|d| format!("Some({d})")).unwrap_or_else(|| "None".into());
    writeln!(
        code,
