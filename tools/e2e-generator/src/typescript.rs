...
pub fn generate(fixtures: &[Fixture], output_root: &Utf8Path) -> Result<()> {
    ...
    if !embed_fixtures.is_empty() {
        ...
        fs::write(ts_impl_dir.join("embed.spec.ts"), content)
            .context("Failed to write TypeScript embed test file")?;
    }
...
fn generate_embed_tests_ts(fixtures: &[&Fixture]) -> Result<String> {
    ...
    writeln!(buf, "// To regenerate: cargo run -p kreuzberg-e2e-generator -- generate --lang typescript\n")?;
...
fn render_embed_test_ts(fixture: &Fixture) -> Result<String> {
    ...
    writeln!(code, "            normalize: {},", if normalize { "true" } else { "false" })?;
    ...
    if spec.config.get("model").and_then(|m| m.get("name")).is_some_and(|n| n.as_str() == Some("invalid")) {
        if is_async {
            writeln!(code, "        await expect(embed({texts_literal}, config)).rejects.toThrow();")?;
        } else {
            writeln!(code, "        expect(() => embedSync({texts_literal}, config)).toThrow();")?;
        }
        writeln!(code, "    }}, TEST_TIMEOUT_MS);\n")?;
        return Ok(code);
    } else {
        writeln!(code, "        const result = {maybe_await}{func}({texts_literal}, config);")?;
    }

    let count_lit = assertions.count.map(|c| c.to_string()).unwrap_or_else(|| "undefined".to_string());
    let dims_lit = assertions.dimensions.map(|d| d.to_string()).unwrap_or_else(|| "undefined".to_string());
    writeln!(code, "        assertEmbedResult(result, {count_lit}, {dims_lit}, {}, {}, {}, {});",
