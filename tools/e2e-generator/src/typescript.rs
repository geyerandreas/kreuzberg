...
export function buildConfig(raw: unknown): ExtractionConfig {
    ...
    if (isPlainRecord(source.email)) {
        ...
        target.email = typeof email.msg_fallback_codepage === "number"
...
pub fn generate(fixtures: &[Fixture], output_root: &Utf8Path, mode: &GenerationMode) -> Result<()> {
    ...
    if !embed_fixtures.is_empty() {
        ...
        sorted.sort_by(|a, b| a.id.cmp(&b.id));
        let content = generate_embed_tests_ts(&sorted)?;
        fs::write(ts_impl_dir.join("embed.spec.ts"), content).context("Failed to write TypeScript embed test file")?;
    }
...
pub fn generate_parity(manifest: &ParityManifest, output_root: &Utf8Path, _mode: &GenerationMode) -> Result<()> {
    ...
    writeln!(buf)?;
    writeln!(buf, "import {{ describe, expect, it }} from \"vitest\";")?;
    writeln!(buf, "import {{ extractFileSync }} from \"@kreuzberg/node\";")?;
    writeln!(buf, "import {{ resolveDocument }} from \"./helpers\";")?;
    writeln!(buf)?;
    ...
    // Generate a describe block for ExtractionResult
    for (type_name, fields) in &struct_types {
        ...
        let required_fields: Vec<String> = fields
            ...
            .map(|(name, _)| parity::to_camel_case(name))
            .collect();
        ...
        writeln!(buf, "\t\t\texpect(result).toHaveProperty(field);")?;
        writeln!(buf, "\t\t}}")?;
        writeln!(buf, "\t}});")?;
        writeln!(buf)?;
        writeln!(buf, "\tit(\"has all known fields\", async () => {{")?;
        writeln!(
            buf,
            "\t\tconst result = await extractFileSync(resolveDocument(\"pdf/fake_memo.pdf\"));"
        )?;
        writeln!(buf, "\t\tconst allFields = [")?;
        for field in &all_fields {
            writeln!(buf, "\t\t\t\"{field}\",")?;
        }
        writeln!(buf, "\t\t];")?;
        writeln!(buf, "\t\tfor (const field of allFields) {{")?;
        writeln!(buf, "\t\t\texpect(result).toHaveProperty(field);")?;
        writeln!(buf, "\t\t}}")?;
        writeln!(buf, "\t}});")?;
        writeln!(buf, "}});")?;
    ...
    // Generate a describe block for ExtractionConfig
    for (type_name, fields) in &struct_types {
        ...
        writeln!(buf, "\t\t];")?;
        writeln!(buf, "\t\texpect(allFields.length).toBeGreaterThan(0);")?;
        writeln!(buf, "\t}});")?;
        writeln!(buf, "}});")?;
        writeln!(buf)?;
    }

    // Generate describe blocks for other struct types
    for (type_name, fields) in &struct_types {
        ...
        writeln!(buf, "describe(\"{type_name} field parity\", () => {{")?;
        writeln!(buf, "\tit(\"has expected fields in manifest\", () => {{")?;
    ...
    let path = ts_tests_dir.join("parity.spec.ts");
    fs::write(&path, &buf).with_context(|| format!("Writing {path}"))?;

    Ok(())
}

fn generate_embed_tests_ts(fixtures: &[&Fixture]) -> Result<String> {
    let mut buf = String::new();
    writeln!(buf, "// Code generated by kreuzberg-e2e-generator. DO NOT EDIT.")?;
    writeln!(
        buf,
    ...
    writeln!(buf, "// Tests for standalone embed() / embedSync() fixtures.\n")?;
    writeln!(buf, "import {{ describe, expect, it }} from \"vitest\";")?;
    writeln!(buf, "import {{ assertEmbedResult }} from \"./helpers.js\";")?;
    writeln!(buf, "import {{ embed, embedSync }} from \"@kreuzberg/node\";")?;
    writeln!(buf, "import type {{ EmbeddingConfig }} from \"@kreuzberg/node\";\n")?;
    writeln!(buf, "const TEST_TIMEOUT_MS = 60_000;\n")?;

    writeln!(buf, "describe(\"standalone embed fixtures\", () => {{")?;

    for fixture in fixtures {
        buf.push_str(&render_embed_test_ts(fixture)?);
    }
...
fn render_embed_test_ts(fixture: &Fixture) -> Result<String> {
    let mut code = String::new();
    ...
    // vitest `it` syntax
    let test_name = format!("test_{}", to_kebab_case(&fixture.id).replace("-", "_"));
    if is_async {
        writeln!(code, "    it(\"{test_name}\", async () => {{")?;
    } else {
        writeln!(code, "    it(\"{test_name}\", () => {{")?;
    }
    writeln!(code, "        // {}", escape_ts_string(&fixture.description))?;
    ...
    if !skip_platforms.is_empty() {
        let conditions: Vec<String> = skip_platforms
            ...
            .filter_map(|p| match p.as_str() {
                "x86_64-pc-windows-msvc" => Some("process.arch === \"x64\" && process.platform === \"win32\""),
                "aarch64-apple-darwin" => Some("process.arch === \"arm64\" && process.platform === \"darwin\""),
                _ => None,
            })
            .map(String::from)
            .collect();
        if !conditions.is_empty() {
            let combined = conditions.join(" || ");
            writeln!(code, "        if ({combined}) {{ return; }}")?;
        }
    }
    ...
    if let Some(bs) = batch_size {
        writeln!(code, "            batchSize: {bs},")?;
    }
    writeln!(code, "        }};")?;
    writeln!(code)?;

    let texts_literal = render_ts_string_list(&spec.texts);
    let maybe_await = if is_async { "await " } else { "" };
    let func = if is_async { "embed" } else { "embedSync" };
        ...
        .is_some_and(|n| n.as_str() == Some("invalid"))
    {
        if is_async {
            writeln!(
                code,
                "        await expect(embed({texts_literal}, config)).rejects.toThrow();"
            )?;
        } else {
            writeln!(
                code,
                "        expect(() => embedSync({texts_literal}, config)).toThrow();"
            )?;
        }
        writeln!(code, "    }}, TEST_TIMEOUT_MS);\n")?;
        return Ok(code);
    } else {
        writeln!(
            code,
            "        const result = {maybe_await}{func}({texts_literal}, config);"
        )?;
    }
    ...
    writeln!(
        code,
        "        assertEmbedResult(result, {count_lit}, {dims_lit}, {}, {}, {}, {});",
...
fn render_ts_string_list(items: &[String]) -> String {
    let parts: Vec<String> = items.iter().map(|s| format!("\"{}\"", escape_ts_string(s))).collect();
    format!("[{}]", parts.join(", "))
}
