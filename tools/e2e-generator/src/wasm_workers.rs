...
fn generate_embed_tests(fixtures: &[Fixture], output_dir: &Utf8Path) -> Result<()> {
    let embed_fixtures: Vec<_> = fixtures
        .iter()
        .filter(|f| {
            f.is_embed() && crate::fixtures::should_include_for_wasm(f, WasmTarget::Workers)
        })
        .collect();
    ...
    writeln!(buffer)?;
    writeln!(buffer, "import {{ describe, it }} from \"vitest\";")?;
    writeln!(buffer, "import {{ embedTexts, initWasm }} from \"@kreuzberg/wasm\";")?;
    writeln!(
        buffer,
        "import {{ assertions, buildConfig }} from \"./helpers.js\";"
    )?;
    writeln!(buffer)?;
    writeln!(buffer, "describe(\"Embeddings\", () => {{")?;

    for fixture in embed_fixtures {
        buffer.push_str(&render_embed_test_workers(fixture)?);
    }

    writeln!(buffer, "}});")?;

    let path = output_dir.join("embeddings.spec.ts");
    fs::write(&path, buffer).with_context(|| format!("Writing {}", path))?;

    Ok(())
}

fn render_embed_test_workers(fixture: &Fixture) -> Result<String> {
    let mut buffer = String::new();
    ...
    writeln!(buffer, "    it(\"{}\", async () => {{", escape_ts_string(&title))?;
