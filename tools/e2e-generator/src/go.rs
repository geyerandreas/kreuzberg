use crate::fixtures::{
    Assertions, EmbedAssertions, EmbedSpec, ExtractionMethod, Fixture, InputType, RenderAssertions,
};
...
fn generate_embed_tests_go(fixtures: &[&Fixture]) -> Result<String> {
    ...
    writeln!(buf, "// To regenerate: cargo run -p kreuzberg-e2e-generator -- generate --lang go\n")?;
...
fn render_embed_test_go(fixture: &Fixture) -> Result<String> {
    ...
    if !skip_platforms.is_empty() {
        let conditions: Vec<String> = skip_platforms
            ...
            .filter_map(|p| match p.as_str() {
                "x86_64-pc-windows-msvc" => Some("runtime.GOARCH == \"amd64\" && runtime.GOOS == \"windows\"".to_string()),
    ...
    let model_name = spec.config.get("model").and_then(|m| m.get("name")).and_then(|n| n.as_str()).unwrap_or("balanced");
    let normalize = spec.config.get("normalize").and_then(|v| v.as_bool()).unwrap_or(false);
    let batch_size_lit = spec.config.get("batch_size").and_then(|v| v.as_u64()).map(|bs| bs.to_string()).unwrap_or_else(|| "0".to_string());

    writeln!(code, "\tconfig := &kreuzberg.EmbeddingConfig{{")?;
    writeln!(code, "\t\tModel: &kreuzberg.EmbeddingModelType{{")?;
    writeln!(code, "\t\t\tType_: \"preset\",")?;
    writeln!(code, "\t\t\tPreset: kreuzberg.String(\"{model_name}\"),")?;
    writeln!(code, "\t\t}},")?;
    if normalize {
        writeln!(code, "\t\tNormalize: kreuzberg.Bool(true),")?;
    }
    if batch_size_lit != "0" {
        writeln!(code, "\t\tBatchSize: kreuzberg.Int({batch_size_lit}),")?;
    }
    writeln!(code, "\t}}")?;
    writeln!(code)?;
    ...
    if spec.config.get("model").and_then(|m| m.get("name")).is_some_and(|n| n.as_str() == Some("invalid")) {
    ...
    let count_lit = assertions.count.map(|c| format!("kreuzberg.Int({c})")).unwrap_or_else(|| "nil".to_string());
    let dims_lit = assertions.dimensions.map(|d| format!("kreuzberg.Int({d})")).unwrap_or_else(|| "nil".to_string());
    writeln!(code, "\tassertEmbedResult(t, result, {count_lit}, {dims_lit}, {}, {}, {}, {})",
