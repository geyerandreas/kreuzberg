...
fn generate_embed_tests(fixtures: &[Fixture], output_dir: &Utf8Path) -> Result<()> {
    ...
    let content = render_embed_category_elixir(&embed_fixtures)?;
    let test_file = output_dir.join("e2e/embed_standalone_test.exs");
    fs::write(&test_file, content).with_context(|| format!("Failed to write {test_file}"))?;
    Ok(())
}

fn render_embed_category_elixir(fixtures: &[&Fixture]) -> Result<String> {
    ...
    writeln!(buffer)?;
    writeln!(buffer, "defmodule E2E.EmbedStandaloneTest do")?;
    ...
    for fixture in fixtures {
        buffer.push_str(&render_embed_test_elixir(fixture)?);
        writeln!(buffer)?;
    }
...
fn render_embed_test_elixir(fixture: &Fixture) -> Result<String> {
    ...
    let texts_list = render_elixir_string_list(&embed.texts);
    let config_expr = if !embed.config.is_empty() {
        render_embed_config_elixir(&embed.config)?
    } else {
        "nil".to_string()
    };
    ...
    writeln!(code, "      try do")?;
    writeln!(code, "        case Kreuzberg.{func}({texts_list}, {config_expr}) do")?;
    writeln!(code, "          {{:ok, res}} -> res")?;
    writeln!(code, "          _ -> flunk(\"Embedding failed\")")?;
    writeln!(code, "        end")?;
    writeln!(code, "      rescue")?;
    writeln!(code, "        _ -> flunk(\"Embedding failed (exception)\")")?;
    writeln!(code, "      end")?;
