...
fn generate_embed_tests_ruby(fixtures: &[&Fixture]) -> Result<String> {
    ...
    writeln!(buf, "# To regenerate: cargo run -p kreuzberg-e2e-generator -- generate --lang ruby\n")?;
...
fn render_embed_test_ruby(fixture: &Fixture) -> Result<String> {
    ...
    if !skip_platforms.is_empty() {
        let conditions: Vec<String> = skip_platforms
            ...
            .filter_map(|p| match p.as_str() {
                ...
                "aarch64-apple-darwin" => Some("RUBY_PLATFORM.match?(/darwin/) && RUBY_PLATFORM.match?(/arm64|aarch64/)".to_string()),
                _ => None,
            })
            .collect();
    ...
    if spec.config.get("model").and_then(|m| m.get("name")).is_some_and(|n| n.as_str() == Some("invalid")) {
        writeln!(code, "    expect {{ Kreuzberg.{method_call}(texts: texts, config: config) }}.to raise_error(StandardError)")?;
        writeln!(code, "  end\n")?;
        return Ok(code);
    } else {
        writeln!(code, "    result = Kreuzberg.{method_call}(texts: texts, config: config)")?;
    }

    let count_lit = assertions.count.map(|c| c.to_string()).unwrap_or_else(|| "nil".to_string());
    let dims_lit = assertions.dimensions.map(|d| d.to_string()).unwrap_or_else(|| "nil".to_string());
    writeln!(code, "    E2ERuby::Assertions.assert_embed_result(result, count: {count_lit}, dimensions: {dims_lit}, no_nan: {}, no_inf: {}, non_zero: {}, normalized: {})",
        assertions.no_nan,
        assertions.no_inf,
        assertions.non_zero,
