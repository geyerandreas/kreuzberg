...
pub fn generate(fixtures: &[Fixture], output_root: &Utf8Path, mode: &GenerationMode) -> Result<()> {
    ...
    if !embed_fixtures.is_empty() {
        ...
        sorted.sort_by(|a, b| a.id.cmp(&b.id));
        let content = generate_embed_tests_ruby(&sorted)?;
        fs::write(spec_dir.join("embed_spec.rb"), content).context("Failed to write Ruby embed spec file")?;
    }
...
fn generate_embed_tests_ruby(fixtures: &[&Fixture]) -> Result<String> {
    let mut buf = String::new();
    writeln!(buf, "# frozen_string_literal: true\n")?;
    ...
    writeln!(buf, "# rubocop:disable Metrics/BlockLength")?;
    writeln!(buf, "require_relative 'spec_helper'\n")?;
    writeln!(buf, "RSpec.describe 'embed fixtures' do")?;

    for fixture in fixtures {
        buf.push_str(&render_embed_test_ruby(fixture)?);
    }

    writeln!(buf, "end")?;
    writeln!(buf, "# rubocop:enable Metrics/BlockLength")?;
    Ok(buf)
}

fn render_embed_test_ruby(fixture: &Fixture) -> Result<String> {
    let mut code = String::new();
    let spec = fixture.embed_spec();
    let assertions = fixture.embed_assertions();
    let is_async = spec.async_variant;

    writeln!(code, "  it {} do", render_ruby_string(&fixture.id))?;
    ...
    if !skip_platforms.is_empty() {
        let conditions: Vec<String> = skip_platforms
            ...
            .filter_map(|p| match p.as_str() {
                "x86_64-pc-windows-msvc" => Some("Gem.win_platform?".to_string()),
                "aarch64-apple-darwin" => {
                    Some("RUBY_PLATFORM.match?(/darwin/) && RUBY_PLATFORM.match?(/arm64|aarch64/)".to_string())
                }
                _ => None,
            })
            .collect();
        if !conditions.is_empty() {
            ...
            writeln!(code, "    skip('Platform restricted') if {combined}")?;
        }
    }
    ...
    writeln!(code, "    config = {{")?;
    writeln!(code, "      model: {{ type: \"preset\", name: \"{model_name}\" }},")?;
    writeln!(code, "      normalize: {normalize},")?;
    if let Some(bs) = batch_size {
        writeln!(code, "      batch_size: {bs}")?;
    }
    writeln!(code, "    }}")?;
    writeln!(code)?;

    let texts_literal = render_string_array(&spec.texts);
    writeln!(code, "    texts = {texts_literal}")?;

    let method_call = if is_async { "embed" } else { "embed_sync" };
        ...
        .is_some_and(|n| n.as_str() == Some("invalid"))
    {
        writeln!(
            code,
            "    expect {{ Kreuzberg.{method_call}(texts: texts, config: config) }}.to raise_error(StandardError)"
        )?;
        writeln!(code, "  end\n")?;
        return Ok(code);
    } else {
        writeln!(
            code,
            "    result = Kreuzberg.{method_call}(texts: texts, config: config)"
        )?;
    }
    ...
    writeln!(
        code,
        "    E2ERuby::Assertions.assert_embed_result(result, count: {count_lit}, dimensions: {dims_lit}, no_nan: {}, no_inf: {}, non_zero: {}, normalized: {})",
        assertions.no_nan, assertions.no_inf, assertions.non_zero, assertions.normalized
    )?;
    writeln!(code, "  end\n")?;
