...
pub mod assertions {
    ...
    pub fn assert_structured_output(
        ...
        if let Some(true) = validates_schema {
            ...
            assert!(val.is_object() || val.is_array(), "Expected structured_output to be a JSON object or array");
        }
        if let Some(fields) = field_exists {
            let val = output.expect("structured_output required for field_exists assertion");
            let obj = val.as_object().expect("structured_output must be a JSON object for field_exists");
            for field in fields {
                assert!(
