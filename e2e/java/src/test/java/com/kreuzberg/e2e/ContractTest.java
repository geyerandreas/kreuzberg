...
public class ContractTest {
    ...
    public void apiBatchFileWithTimeoutSync() throws Exception {
        ...
        if (!Files.exists(documentPath)) {
            String msg = String.format("Skipping api_batch_file_with_timeout_sync: missing document at %s", documentPath);
            System.err.println(msg);
            org.junit.jupiter.api.Assumptions.assumeTrue(false, msg);
            return;
        }
        ...
        try {
            ...
            String skipReason = E2EHelpers.skipReasonFor(e, "api_batch_file_with_timeout_sync", Collections.emptyList(), null);
            if (skipReason != null) {
                org.junit.jupiter.api.Assumptions.assumeTrue(false, skipReason);
                return;
            }
    ...
    public void configChunkingPrependHeadingContext() throws Exception {
        JsonNode config = MAPPER.readTree("{\"chunking\":{\"chunker_type\":\"markdown\",\"max_chars\":300,\"max_overlap\":50,\"prepend_heading_context\":true}}");
        E2EHelpers.runFixture(
    ...
    public void configForceOcrPages() throws Exception {
        JsonNode config = MAPPER.readTree("{\"force_ocr_pages\":[1],\"ocr\":{\"backend\":\"tesseract\",\"language\":\"eng\"}}");
        E2EHelpers.runFixture(
    ...
    public void configPdfAnnotationsCount() throws Exception {
        ...
        if ((System.getProperty("os.arch").equals("aarch64") && System.getProperty("os.name").startsWith("Linux"))) {
            org.junit.jupiter.api.Assumptions.assumeTrue(false, "Skipping config_pdf_annotations_count: not supported on this platform");
            return;
        }
        E2EHelpers.runFixture(
