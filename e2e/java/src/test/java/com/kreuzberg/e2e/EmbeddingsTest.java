...
public class EmbeddingsTest {
    ...
    public void embeddingAsync() throws Exception {
        ...
        if ((System.getProperty("os.arch").equals("amd64") && System.getProperty("os.name").startsWith("Windows"))) {
            org.junit.jupiter.api.Assumptions.assumeTrue(false, "Skipping embedding_async: not supported on this platform");
            return;
        }
        ...
        try {
            ...
            if (cause instanceof Exception) {
                String skipReason = E2EHelpers.skipReasonFor((Exception) cause, "embedding_async", Arrays.asList("embeddings"), "Embeddings not supported on Windows");
                if (skipReason != null) {
    ...
    public void embeddingBalancedPreset() throws Exception {
        JsonNode config = MAPPER.readTree("{\"chunking\":{\"embedding\":{\"model\":{\"name\":\"balanced\",\"type\":\"preset\"},\"normalize\":true},\"max_chars\":500,\"max_overlap\":50}}");
        if ((System.getProperty("os.arch").equals("amd64") && System.getProperty("os.name").startsWith("Windows"))) {
            org.junit.jupiter.api.Assumptions.assumeTrue(false, "Skipping embedding_balanced_preset: not supported on this platform");
            return;
        }
    ...
    public void embeddingFastPreset() throws Exception {
        ...
        if ((System.getProperty("os.arch").equals("amd64") && System.getProperty("os.name").startsWith("Windows"))) {
            org.junit.jupiter.api.Assumptions.assumeTrue(false, "Skipping embedding_fast_preset: not supported on this platform");
            return;
        }
        E2EHelpers.runFixture(
